...
namespace HMAC
{
    ...
    class SHA_3
    {
        ...
        string ToBinaryString(string text) => ToBinaryString(Encoding.ASCII.GetBytes(text));
        string ToBinaryString(byte[] byteArray) => string.Join("", byteArray.Select(n => Convert.ToString(n, 2).PadLeft(8, '0')));
        ...
        private BitArray ByteArrayToBitArray(ref byte[] byteArray)
        {
            BitArray State = new BitArray(byteArray.Length * 8);
            for (int i = 0; i < byteArray.Length; ++i)
                for (int j = 7; j >= 0; --j)
                    State[i * 8 + (7 - j)] = (byteArray[i] << j) == 1; //State[i * 8 + j];
            return State;
        }
        ...
        public byte[] GetHashSHA3(string Message)
        {
            string mes = ToBinaryString(Message);
            return SPONGE_BOB(mes + "01", 256);
        }
        public byte[] GetHashSHA3(byte[] Message)
        {
            string mes = ToBinaryString(Message);
            return SPONGE_BOB(mes + "01", 256);
        }
