...
namespace SHA_3
{
    ...
    class SHA_3
    {
        ...
        private string FormResult(ref BitArray State, int limit)
        {
            Console.WriteLine();
            int sum;
            string resultString = "";
            ...
            sum = (State[0]) ? 8 : 0;
            for (int i = 1; i < limit/*State.Count*/; ++i)
            {
                if (i % 4 == 0)
                {
                    resultString += Convert.ToString(sum, 16).ToUpper();
                    sum = 0;
                }
                sum += (State[i]) ? (8 >> (i % 4)) : 0;
            }
        ...
        private string SPONGE_BOB(string message, int OutputBits)
        {
            ...
            string result = FormResult(ref state, OutputBits);
            return result;
        }
        public string GetHashSHA3(string Message, bool isRawInput = false)
        {
